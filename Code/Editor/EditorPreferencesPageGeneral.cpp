...
void CEditorPreferencesPage_General::Reflect(AZ::SerializeContext& serialize)
{
    ...
    if (editContext)
    {
        editContext->Class<GeneralSettings>("General Settings", "General Editor Preferences")
            ...
            ->DataElement(AZ::Edit::UIHandlers::ComboBox, &GeneralSettings::m_editorTheme, "Editor Theme", "Editor theme (discovered from the themes pool on disk)")
                ->Attribute(AZ::Edit::Attributes::StringList, &GeneralSettings::GetThemeList)
            ->DataElement(
...
// THEME DISCOVERY
// ------------------------------------------------------------------------------------
AZStd::vector<AZStd::string> CEditorPreferencesPage_General::GeneralSettings::GetThemeList() const
{
    AZStd::vector<AZStd::string> themeNames;
    const auto themes = AzQtComponents::StyleManager::availableThemes();
    for (const auto& theme : themes)
    {
        themeNames.push_back(theme.folderName.toUtf8().constData());
    }
    return themeNames;
}
