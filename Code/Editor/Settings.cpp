...
void SEditorSettings::Save(bool isEditorClosing)
{
    ...
    SaveValue("Settings", "EditorTheme", QString::fromUtf8(gui.editorTheme.c_str()));
...
void SEditorSettings::Load()
{
    ...
    {
        QString editorThemeStr = QString::fromUtf8(gui.editorTheme.c_str());
        LoadValue("Settings", "EditorTheme", editorThemeStr);
        gui.editorTheme = editorThemeStr.isEmpty()
            ? AZStd::string("O3DE_Original")
            : AZStd::string(editorThemeStr.toUtf8().constData());
    }
