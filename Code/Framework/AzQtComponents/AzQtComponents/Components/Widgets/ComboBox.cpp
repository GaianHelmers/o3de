...
namespace AzQtComponents
{
            ...
            else if (auto itemView = qobject_cast<QAbstractItemView*>(watched))
            {
                ...
                switch (event->type())
                {
                    case QEvent::Show:
                    {
                        ...
                        // background-color does NOT override -> the dropdown renders black. Theme the
                        // view (and its viewport) palette from the active theme's menu background.
                        const QColor popupBg = ConfigHelpers::themeColor("MenuBackgroundColor", QColor(0x22, 0x22, 0x22));
    ...
    protected:
        void initStyleOption(QStyleOptionViewItem* option, const QModelIndex& index) const override
        {
            ...
            // the check column and paints an empty check-box frame for it. drawItemCheckIndicator only
            // controls the checked glyph, so the empty frames have to be gated off here.
            if (option && m_combo && m_combo->view()->selectionMode() == QAbstractItemView::SingleSelection)
            {
                ...
                {
                    // Multi-state combos: per-row check state from the model.
                ...
                else
                {
                    ...
                    // margin in paint() below (so all text stays aligned).
                    option->features &= ~QStyleOptionViewItem::HasCheckIndicator;
        ...
        QSize sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const override
        {
            ...
            // and popup placement -> they stay in sync. Tokenised so it is tunable from the theme.
            size.setHeight(qMax(size.height(), ConfigHelpers::themeInt("SizeComboItemHeight", 24)));
            return size;
        }
